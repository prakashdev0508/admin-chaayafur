...
/** jsPDF Helvetica cannot render many Unicode glyphs from IG captions. */
...
export function downloadInstagramInsightsPdf(data: InstagramMediaInsights): void {
  ...
  const caption = pdfSafeText(
    (data.media?.caption?.trim() || "Untitled reel").slice(0, 280),
  );
  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(10);
  pdf.setTextColor(...INK);
  ...
  y += 5;
  pdf.text(`Type: ${pdfSafeText(data.media?.media_type ?? "-")}`, margin, y);
  y += 5;
