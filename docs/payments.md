...
- On successful payment: order → `CONFIRMED`, **Performa** invoice generated
- On failed/expired payment: order → `PAYMENT_FAILED`, stock restored
...
- **MANUAL** payments are created by `POST /admin/orders` and completed with `POST /admin/orders/:id/mark-paid` (no Razorpay webhook)

### Payment statuses

| Status | Description |
|--------|-------------|
| `PENDING` | Awaiting Razorpay payment (checkout) or offline collection (manual orders) |
| `COMPLETED` | Payment received via Razorpay or staff mark-paid |
...
| Method | When |
|--------|------|
| `RAZORPAY` | Customer checkout and customization convert-to-order |
| `MANUAL` | Staff `POST /admin/orders` — cash / UPI / bank transfer recorded via mark-paid |
...
payment_link.paid
    → Payment COMPLETED, Order CONFIRMED, Performa invoice generated

payment_link.expired / payment.failed / payment_link.cancelled
    → Payment FAILED, Order PAYMENT_FAILED, stock restored
```
